...
export class ChatSendCustomOfferComponent {
  ...
  handleCountryChange(val: string) {
    this.service.fulfillmentMethod['address']['country'] = val;
  }
  ...
  onDeliveryMethod(option: string) {
    ...
    methodOptions.map(key => {
      if (key !== 'address' && key !== 'parcel' && key !== 'localServiceRadius' && key !== 'localServiceRadiusUom' && key !== option) {
  ...
  ngOnChanges(changes) {
    ...
    if (changes.linkedService && changes.linkedService.currentValue) {
      ...
      if (changes.linkedService.currentValue.isCustomRequest) {
        delete serviceToConvert.service['media'][0]['_id'];
        serviceData['media'] = [serviceToConvert.service['media'][0]];
      }
  ...
  preventClick = () => {
    ...
    sendService() {
      ...
      if (serviceToSend['fulfillmentMethod']['shipment']) {
        if (typeof serviceToSend['fulfillmentMethod']['address']['state'] === 'object') {
          let val = serviceToSend['fulfillmentMethod']['address']['state']['value'];
          serviceToSend['fulfillmentMethod']['address']['state'] = val;
        }
      }
