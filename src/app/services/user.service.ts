...
export class UserService {
  ...
  getSellerEligibility(): Observable<UserSellerEligibility> {
    const url = `${this.apiUrl}seller-eligibility`;

    return this.http.get(url).map(res => {

      // transforming "not started" to bool
      res['paymentSetupStatus'] = (res['paymentSetupStatus'].toLowerCase() !== 'not started');
      res['backgroundCheckStatus'] = (res['backgroundCheckStatus'].toLowerCase() !== 'not started');

      return res as UserSellerEligibility;
    });
  }
  ...
  getsuggestedFB(skip,limit) {
    const url = `${environment.apiUrl}users/invite-suggested/fb?skip=${skip}&limit=${limit}`;
    const httpOptions = {
      headers: new HttpHeaders({
  ...
  getsuggestedFollow(skip,limit) {
    const url = `${environment.apiUrl}users/invite-suggested?skip=${skip}&limit=${limit}`;
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${this.authService.token}`
  ...
  inviteByEmail(payload){
    const url = `${environment.apiUrl}user/invite`;
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${this.authService.token}`
