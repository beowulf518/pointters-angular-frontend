...
export class ConversationService {
  ...
  */
  ...
  getOrdersWithUser(userId) {
    const url = `${this.apiUrl}orders/chat`;
    const httpOptions = {
      headers: new HttpHeaders({
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${this.authService.token}`
    ...
    return new Promise((resolve,reject)=>{
      this.http.get(url+"?userId="+userId, httpOptions)
        .subscribe(res=>{console.log('123',res);resolve(res)},err=>{reject(err)});
    })
  }
  getOffersWithUser(userId) {
  ...
  getMoreOffersWithUser(urlToken) {
    ...
    //return 
    return new Promise((resolve,reject)=>{
      this.http.get(url+urlToken.substr(1), httpOptions)
        .subscribe(res=>{console.log('123',res);resolve(res)},err=>{reject(err)});
    })
  }
  getReuqestsWithUser(userId) {
