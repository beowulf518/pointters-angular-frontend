...
export class ChatComponent implements OnInit {
  ...
  ngOnInit() {
    console.log('offerToSend',this.offerToSend)
    this.conversationService.initSocket();

    // Timere to update conversations
    timeInterval = setInterval(()=>{
      let buffer = this.conversationBuffer.slice(0),promissarray=[];
      buffer.unshift(undefined);
      buffer.map((element)=>{
        promissarray.push(new Promise((resolve,reject)=>{
          this.conversationService.get(element)
          .subscribe(
            (data: any) => {
              resolve(data);
            },err=>{
      ...
      Promise.all(promissarray).then(
        data=>{
          ...
          data.map((element,index)=>{
            metaconvers = metaconvers.concat(data[index].docs)
          })
          if(this.conversations) this.conversations.docs=metaconvers;
        }
      )
    },2000)
    ...
    this.userService.user.subscribe(user => {
      this.user = user;
      this.checkServiceStatus(user._id);
  ...
  checkServiceStatus = (id) => {
    this.serviceService.getServices(id)
      .first()
      .subscribe((res) => {
        if (res) {
  ...
  showRequest = (requestId:string)=>{
    ...
    .subscribe(
      res=>{
        console.log('this.requestService.get ', res)
        this.offerToSend.description = res['description'];
        this.linkedService = {service:res['service'],isCustomRequest:true};
        this.switchTab(1);
      },
      err=>{console.log('this.requestService.get ', err)}
    )
  ...
  loadMoreConversations() {
    if (this.searchMode && this.activeQuery) {
      ...
      if(!this.conversationBuffer.includes(this.conversations['lastDocId'])){
        this.conversationBuffer.push(this.conversations['lastDocId']); 
      }
  ...
  sendChatMessage(media?, mediaIndex?) {
    ...
    let messageData = {
      ...
      "messageText": txt || "",
      "userId": this.conversationPartner.userId||this.conversationPartner.id
    }
    if (this.medias) {
